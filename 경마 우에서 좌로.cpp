...
//        float x = boxPositions[i]; // 각 네모의 x 좌표
//        float y = 480 - (i + 1) * (boxSize + 50); // 각 네모 간 50픽셀 간격으로 위에서 아래로 위치 설정
...
//        // 클릭한 위치의 Y 좌표를 변환 (OpenGL 좌표계는 원점이 왼쪽 하단)
//        float clickY = 480 - Y; // Y 좌표 변환
//
//        // 네모의 범위 안에 클릭한 위치가 있는지 확인
//        for (int i = 0; i < numberOfBoxes; i++) {
//            float boxY = 480 - (i + 1) * (boxSize + 50); // 각 네모의 Y 좌표 계산
//            if (clickY >= boxY && clickY <= boxY + boxSize && !boxFinish[i]) {
//                // 클릭한 Y좌표가 네모의 범위 안에 있을 경우
//                boxPositions[i] = (float)X; // 클릭한 X 좌표로 네모의 X 좌표 변경
//                break; // 첫 번째 네모만 변경 후 루프 종료
//            }
//        }
...
//    if (is_running) {
//        // 0.01초마다 임의의 네모 하나를 오른쪽으로 1픽셀 이동
//        int randomBox = rand() % numberOfBoxes; // 0부터 numberOfBoxes-1 사이의 임의의 네모 선택
//        if (boxPositions[randomBox] < 0) { //끝에 도달하면 위치 초기화
//            boxPositions[randomBox] = 630.0f;
//            boxFinish[randomBox] = 1;
//        }
//        if (!boxFinish[randomBox]) //완주하지 못한 네모만 이동 x 좌표를 1픽셀 증가
//        {
